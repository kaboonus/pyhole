window.addEvent('domready', function() {
	...
	function drawNode(node, x, y) {
		...
		if (node.connections) {
			for (var i = 0; i < node.connections.length; i++) {
				...
				drawLink(x, y, x + 150, y + newLines * rowHeight, child.eol, child.mass, child.adjacent);
	...
	function drawLink(x1, y1, x2, y2, eol, mass, gate) {
		var color;
		if (gate)
			color = '#040'
	...
	function handleClick(system) {
		...
		if (system.connections) {
			var conns = system.connections.each(function(conn) {
				...
				if (conn.adjacent) {
					var note = new Element('span', {'html':'(gate connection)', 
